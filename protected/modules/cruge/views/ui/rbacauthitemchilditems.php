<?php
	...
	// titulos
	echo "<h4 class='center-align'>".ucfirst($model->name)." (".CrugeTranslator::t($rbac->getAuthItemTypeName($model->type)).")</h4>";
	echo "<h3 class='hint'>".$model->description."</h3>";
	echo "<p>".ucfirst(CrugeTranslator::t("haga click en un item para activarlo o desactivarlo"))."</p>";

	$iconPin = Yii::app()->user->ui->getResource('pin.png');
	$imgPin = "<img class='pin-on' src='{$iconPin}' title='".CrugeTranslator::t("Click para asignar/desasignar el item")."'>";
	...
	if((count($roles) > 0) && ($model->type != CAuthItem::TYPE_TASK)){
		foreach($roles as $item){
			...
			$treeDataRoles[] = array(
				'id'=>$item->name,
				'text'=>"<span class='{$asignado} {$loop}'>".$item->name."</span>".$imgPin,
				'htmlOptions'=>array('class'=>'authitem','alt'=>$item->name),
			);
		}
	}
	...
	if(count($tareas) > 0){
		...
		if(count($taskinfo['topmenu']) > 0)
		{
			foreach($taskinfo['topmenu'] as $topTask){
				...
				foreach($taskinfo['childmenu'][$topTask->name] as $child){
					$asignado		= isset($childrens[$child->name]) ? 'checked' : '';
					$loop			= $rbac->detectLoop($model->name,$child->name) ? "loop" : "";
					$hasChildren	= true;
					$children[] 	= array(
						'id'=>$child->name,
						'text'=>
								"<span class='itemchildtext authitemsub {$asignado} {$loop}'>"
									.$rbac->getTaskText($child).
								"</span>".$imgPin,
						'htmlOptions'=>array(
							'class'=>'authitemchild',
							'alt'=>$child->name,
				...
				$loop			= $rbac->detectLoop($model->name,$topTask->name) ? "loop" : "" ;
				$treeDataMenu[] = array(
					'id'=>$topTask->name,
					'text'=>
							"<span class='itemtext authitemtop {$asignado} {$loop}'>"
								.$text.
							"</span>".$imgPin,
					'expanded'=>false,
		...
		if(count($taskinfo['orphan'])>0)
			foreach($taskinfo['orphan'] as $orpTask){
				...
				$loop				= $rbac->detectLoop($model->name,$orpTask->name) ? "loop" : "";
				$treeDataError[]	= array(
					'id'=>$orpTask->name,
					'text'=>
							"<span class='{$asignado} {$loop}'>"
								.$rbac->getTaskText($orpTask).
							"</span>".$imgPin,
					'expanded'=>false,
					'hasChildren'=>false,
					'htmlOptions'=>array(
		...
		if(count($taskinfo['regular'])>0)
			foreach($taskinfo['regular'] as $task){
				...
				$loop				= $rbac->detectLoop($model->name,$task->name) ? "loop" : "";
				$treeDataRegular[]	= array(
					'id'=>$task->name,
					'text'=>
							"<span class='{$asignado} {$loop}'>"
	...
	if(count($operaciones) > 0){
		...
		foreach($listacatg as $catg_filter => $catg_name){
			$childs = array();
			foreach($rbac->getOperationsFiltered($catg_filter, $operaciones) as $item){
	...
	$this->widget('CTreeView',array(
		...
		'data'=>array(
			...
			// OPERATIONS  TREENODE
			array(
				'text'=>"<b>".CrugeTranslator::t("Operaciones Organizadas por Tipo")."</b>",
				'expanded'=>true,
...
<div class="center-align">
	<div class="col s6">
		<div class="btn waves-effect waves-ligh red accent-5">
			...
			<?php echo CHtml::Button('Volver', array('class'=>'white-text','onclick' => 'js:document.location.href="../../../ui/rbaclistroles"')); ?>
		</div>
	</div>
</div>
